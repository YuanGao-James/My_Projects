...
Usage - sources:
    $ python path/to/yolo5.py --weights yolov5s.pt --source 0              # webcam
                                                             img.jpg        # image
                                                             vid.mp4        # video
                                                             path/          # directory
                                                             path/*.jpg     # glob
...
Usage - formats:
    $ python path/to/yolo5.py --weights yolov5s.pt                 # PyTorch
...
def run(
        ...
        data=ROOT / 'CW.yaml',  # dataset.yaml path
    ...
    # Dataloader
    dataset = LoadImages(img, path=path, img_size=imgsz, stride=stride, auto=pt)
    bs = 1  # batch_size
...
def predict():
    if request.method == 'POST':
        ...
        imBytes = np.frombuffer(img, np.uint8)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/CW.yaml', help='(optional) dataset.yaml path')

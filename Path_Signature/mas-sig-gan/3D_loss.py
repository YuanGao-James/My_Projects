...
def main(b,c,d):
    ...
    parser.add_argument("--num-samples", type=List, default=[7, 5, 3], help="the num of traders. [noise, technical, fundamental]")
    ...
    parser.add_argument("--param-d", type=float, default=0.5, help="tbbbbhe param d of mas to fit")
    parser.add_argument("--param-init", type=bool, default=True, help="init param or random")
    parser.add_argument("--param-b-init", type=float, default=b, help="the init param b of mas")
    parser.add_argument("--param-c-init", type=float, default=c, help="the init param c of mas")
    parser.add_argument("--param-d-init", type=float, default=d, help="the init param d of mas")
    ...
    dataset = MASDataset(in_price, in_price_star, in_sigma, sigs_pred)

    return model.train(dataset, task=task)
...
if __name__ == "__main__":
    ...
    y = []
    loss = []
    for b in np.linspace(-1, 2, 30):
        for c in np.linspace(-1, 2, 30):
                x.append(b)
                y.append(c)
                loss.append(float(main(b, c, 0.5)))
    loss = np.array(loss)
    x = np.array(x)
    y = np.array(y)
    fig = plt.figure()
    ax = plt.axes(projection="3d")
    ax.plot_trisurf(x, y, loss, linewidth=0, antialiased=False, cmap='rainbow')
    plt.show()

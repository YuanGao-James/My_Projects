...
def plot_bbox(img_path, img_dir, out_dir, gt=None ,dt=None, cls2label=None, line_thickness=None):
    ...
    cv2.imencode('.jpg', img)[1].tofile(os.path.join(out_dir,img_path))
...
def plot_box_on_pic(root):
    ...
    cls_dir = 'D:/Using AI/烟弹管缺陷检测/烟弹管缺陷检测0428/label.txt'
    cls_dict = {}
    ...
    for each_img in tqdm(os.listdir(img_dir)):
        gt = None
        dt = None
        if os.path.exists(os.path.join(GT_dir, each_img.replace('bmp', 'txt'))):
            gt = os.path.join(GT_dir, each_img.replace('bmp', 'txt'))
        if os.path.exists(os.path.join(DT_dir, each_img.replace('bmp', 'txt'))):
            dt = os.path.join(DT_dir, each_img.replace('bmp', 'txt'))
        # print(each_img,img_dir,out_dir,gt,dt,cls_dict)
        plot_bbox(each_img, img_dir, out_dir, gt, dt, cls2label=cls_dict)


if __name__ == "__main__":
    plot_box_on_pic('D:/Using AI/烟弹管缺陷检测/Data/4.27/A3138MG000_7K01D72PAK00001')
    # plot_box_on_pic('D:/Using AI/烟弹管缺陷检测/Data/4.27/A3138MG000_7K01D72PAK00210')
    # for num in range(1,4):
    #     num = str(num)
    #     root_path = 'D:/Using AI/烟弹管缺陷检测/Data/5.27'
    #     pred_path = 'D:/Using AI/烟弹管缺陷检测/Data/5.27'
    #     img_dir = os.path.join(root_path, 'pic/210/'+num)
    #     GT_dir = os.path.join(root_path, 'labels/210/'+num)
    #     DT_dir = os.path.join(pred_path)
    #     out_dir = os.path.join(root_path, 'outputs_210')
    #     # cls_dir = os.path.join(root_path,'label.txt')
    #     cls_dir = 'D:/Using AI/烟弹管缺陷检测/label.txt'
    #     cls_dict = {}
    #
    #     if not os.path.exists(img_dir):
    #         raise Exception("image dir {} do not exist!".format(img_dir))
